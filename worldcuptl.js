...
`;
  ...
  function normalizeDash(str) {
    if (!str) return '';
    return str.replace(/&#8212;/g, '—').replace(/&mdash;/g, '—');
  }
  ...
  function renderScore(item) {
    ...
    const score = normalizeDash(item.score || '0 — 0');
    const info = item.scoreInfo ? `<span class="tlwc26-score-info">${escapeHtml(normalizeDash(item.scoreInfo))}</span>` : '';
  ...
  function renderDate(item) {
    ...
    return `<div class="tlwc26-date${liveClass}">${icon} ${escapeHtml(normalizeDash(item.date))}</div>`;
  }
