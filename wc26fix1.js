// ===== FIFA WORLD CUP 2026 LIVE WIDGET (TIMEZONE FIX) =====
// Combined JS + CSS + HTML — Single file for GitHub / Blogger
// Source: https://worldcup26.ir (free open API)
//
// === WHAT WAS WRONG ===
// match.local_date from the API is the STADIUM'S local clock time
// (e.g. "06/29/2026 19:00" at a Pacific-time stadium means 7pm Pacific).
// The old code did:
//     new Date(year, month-1, day, hours, minutes)
// which builds a Date using the VISITOR'S browser timezone, not the
// stadium's timezone. So a 7pm Pacific kickoff was silently treated as
// 7pm in whatever timezone the visitor's device is set to — wrong by
// however many hours separate the two zones. The label "ET" was also
// hardcoded regardless of which US/Canada/Mexico timezone the match
// was actually in, and "is this match live/today" checks inherited
// the same offset error.
//
// === THE FIX ===
// 1. Each match now resolves a venue IANA timezone (from match.timezone /
//    match.stadium_timezone if the API provides it, else looked up from
//    a stadium-city table baked into this file as a fallback).
// 2. We convert the "wall clock" local_date + venue timezone into a
//    real UTC instant using a small, dependency-free offset calculator.
// 3. All status checks (live/upcoming/finished) and "is it today" run
//    against that correct UTC instant.
// 4. Display time/date is rendered in the VISITOR'S OWN timezone via
//    Intl/toLocaleString, with the correct zone abbreviation — no more
//    hardcoded "ET".
...
@media (min-width: 768px) {
  ...
  const MATCH_LINK = "https://www.pashtomedium.com/p/fifa-world-cup-2026.html";
  ...
  // ===== TIMEZONE FIX: stadium-city → IANA timezone lookup =====
  // Used as a FALLBACK only if the API response doesn't already include
  // a timezone/stadium_timezone field on the match object. If it does,
  // that field always wins (see resolveVenueTimeZone below).
  ...
  // Figure out which IANA timezone a match's local_date is expressed in.
  // Priority: explicit field from API > stadium/city lookup table > UTC fallback.
  function resolveVenueTimeZone(match) {
    if (match.timezone) return match.timezone;
    if (match.stadium_timezone) return match.stadium_timezone;
    const city = match.city || match.host_city || (match.stadium && match.stadium.city);
    if (city && stadiumTimeZones[city]) return stadiumTimeZones[city];
    // Safe fallback: treat as UTC rather than silently using the
    // visitor's local zone (which was the original bug).
    return "UTC";
  }

  // Get the UTC offset (in minutes) that a given IANA timezone has
  // AT A SPECIFIC WALL-CLOCK MOMENT (needed because of DST).
  // Works by formatting the same instant in both UTC and the target
  // zone and diffing the two, using only built-in Intl — no library.
  function getTimeZoneOffsetMinutes(timeZone, atUtcDate) {
    ...
    const parts = dtf.formatToParts(atUtcDate).reduce((acc, p) => {
      acc[p.type] = p.value;
      return acc;
    }, {});
  ...
  // Convert a "wall clock" date/time that is known to be in `timeZone`
  // into the correct UTC Date object (and therefore correct everywhere,
  // including the visitor's own browser).
  function venueWallClockToUTC(year, month, day, hours, minutes, timeZone) {
    // Initial guess: treat wall clock as if it were UTC, then correct
    // by the target zone's offset at that approximate instant. One
    // correction pass is sufficient because DST transitions are at
    // least an hour, never sub-minute, so this converges.
    const guessUTC = new Date(Date.UTC(year, month - 1, day, hours, minutes));
    const offsetMin = getTimeZoneOffsetMinutes(timeZone, guessUTC);
    return new Date(guessUTC.getTime() - offsetMin * 60000);
  }

  // local_date format from the API: "MM/DD/YYYY HH:mm" (24h, venue-local)
  function parseMatchDateTime(match) {
    const [datePart, timePart] = match.local_date.split(" ");
    const [month, day, year] = datePart.split("/").map(Number);
    const [hours, minutes] = timePart.split(":").map(Number);
    const venueTz = resolveVenueTimeZone(match);
    // Correct UTC instant, derived from the STADIUM's local wall clock,
    // not the visitor's. This is the actual fix.
    return venueWallClockToUTC(year, month, day, hours, minutes, venueTz);
  }

  // Display helpers now render in the VISITOR'S OWN timezone (the
  // browser default), which is what people expect from "kickoff time"
  // unless you explicitly want venue time — and label it correctly
  // instead of hardcoding "ET".
  ...
  // "Today" must also be evaluated in the visitor's local calendar day,
  // compared against the correct UTC instant — not by re-parsing the
  // venue-local string as if it were the visitor's local string.
  ...
  function getMatchStatus(match, matchUtcDate) {
    const now = new Date();
    const matchEnd = new Date(matchUtcDate.getTime() + 2 * 60 * 60 * 1000);
    if (match.finished === "TRUE" || match.time_elapsed === "finished") return "finished";
    if (now >= matchUtcDate && now <= matchEnd) return "live";
    return "upcoming";
  }
  ...
  function renderMatch(match, matchUtcDate, index, total) {
    const container = document.getElementById("wc-match-container");
    const status = getMatchStatus(match, matchUtcDate);
    ...
    container.innerHTML = `
      <a class="wc-match-card wc-fade-in" href="${MATCH_LINK}" target="_blank">
        ...
        <div class="wc-match-meta">
          <span class="wc-date">${formatDate(matchUtcDate)}</span>
          <span class="wc-time">🕐 ${formatTime(matchUtcDate)}</span>
    ...
    if (total > 1) {
      ...
      counter.textContent = `${index + 1} of ${total} matches today`;
    } else {
      counter.style.display = total === 1 ? "block" : "none";
      counter.textContent = total === 1 ? `1 match today` : "";
    }
  }
  ...
  async function fetchData() {
    ...
    // Compute correct UTC instant for every match using venue timezone,
    // then filter to "today" using the VISITOR'S calendar day.
    todaysMatches = allMatches
      .map(m => ({ match: m, utcDate: parseMatchDateTime(m) }))
      .filter(({ utcDate }) => isMatchToday(utcDate))
  ...
  function displayCurrent() {
    ...
    if (todaysMatches.length === 1) {
      renderMatch(todaysMatches[0].match, todaysMatches[0].utcDate, 0, 1);
      document.getElementById("wc-header-title").textContent = "Match of the Day";
    } else {
      const { match, utcDate } = todaysMatches[currentIndex];
      renderMatch(match, utcDate, currentIndex, todaysMatches.length);
      document.getElementById("wc-header-title").textContent = "Matches Today";
    }

    const liveEntry = todaysMatches.find(({ match, utcDate }) => getMatchStatus(match, utcDate) === "live");
    const headerStage = document.getElementById("wc-header-stage");
    headerStage.textContent = liveEntry ? "LIVE NOW" : "WC 2026";
  }
