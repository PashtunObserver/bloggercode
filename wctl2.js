...
`;
  ...
  const homeFlag = item.home
    ? `<img class="tlwc26-flag" src="${getFlagUrl(item.home)}" alt="${escapeHtml(item.homeName || item.home)}" loading="lazy"/>`
    : '';

  const awayFlag = item.away
    ? `<img class="tlwc26-flag" src="${getFlagUrl(item.away)}" alt="${escapeHtml(item.awayName || item.away)}" loading="lazy"/>`
    : '';
  ...
  const score = String(item.score || '0 — 0')
    .replace(/&#8212;|&mdash;|—/g,
      '<span class="tlwc26-score-sep">—</span>');

  const info = item.scoreInfo
    ? `<span class="tlwc26-score-info">${
        escapeHtml(item.scoreInfo)
          .replace(/&#8212;|&mdash;|—/g,'—')
      }</span>`
    : '';
  ...
  return `
    <div class="tlwc26-score">
      ...
      <span class="tlwc26-score-num">${score}</span>
