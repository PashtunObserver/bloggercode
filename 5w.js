...
(function() {
  ...
  function rpw_parseBloggerUrl(url) {
    var match = url.match(/^https?:\/\/([^\/]+)(\/\d{4}\/\d{2}\/[^\/]+\.html)$/);
  ...
  // Utility: Extract thumbnail from post content
  function rpw_extractThumbnail(post) {
    // Try media:thumbnail first
    if (post.media$thumbnail && post.media$thumbnail.url) {
      return post.media$thumbnail.url.replace(/\/s72-c\//, '/s320/');
    }
  ...
  function rpw_showLoading(container) {
    container.innerHTML = 
      '<div class="rpw-container">' +
        '<div class="rpw-heading">' + rpw_config.headingText + '</div>' +
  ...
  // Show error
  function rpw_showError(container, message) {
    container.innerHTML = 
      '<div class="rpw-container">' +
        '<div class="rpw-heading">' + rpw_config.headingText + '</div>' +
  ...
  // Render the widget
  function rpw_renderWidget(container, post, postUrl) {
    var thumbnail = rpw_extractThumbnail(post);
    var title = rpw_truncateTitle(post.title.$t || 'Untitled Post');
    var readTime = rpw_estimateReadTime(post.content ? post.content.$t : '');
    ...
    container.innerHTML = 
      '<div class="rpw-container">' +
        ...
        '<a href="' + postUrl + '" class="rpw-card" target="_blank" rel="noopener noreferrer">' +
          '<img class="rpw-thumbnail" src="' + thumbnail + '" alt="' + title + '" loading="lazy" onerror="this.src=\'' + rpw_config.fallbackImage + '\'">' +
  ...
  function rpw_fetchPost(container, postUrl) {
    rpw_showLoading(container);

    var parsed = rpw_parseBloggerUrl(postUrl);
    if (!parsed) {
      rpw_showError(container, 'Invalid Blogger URL format.');
      return;
    }

    var callbackName = rpw_generateCallbackName();
    var feedUrl = 'https://' + parsed.domain + 
                  '/feeds/posts/default' + parsed.path + 
    ...
    window[callbackName] = function(data) {
      ...
      if (!data || !data.feed || !data.feed.entry || data.feed.entry.length === 0) {
        rpw_showError(container);
        return;
      }

      rpw_renderWidget(container, data.feed.entry[0], postUrl);
    };
  ...
  function rpw_init() {
    ...
    for (var i = 0; i < widgets.length; i++) {
      (function(widget) {
        ...
        var customHeading = widget.getAttribute('data-rpw-heading');
        
        if (customHeading) {
          rpw_config.headingText = customHeading;
        }
        
        if (postUrl) {
          // Small delay to prevent rate limiting if multiple widgets
          setTimeout(function() {
