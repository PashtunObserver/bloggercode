...
/**
 * TOC WIDGET - TABLE OF CONTENTS ENGINE
 * Prefix: toc_ (Table of Contents)
 * Version: 3.0 - Fixed page scroll jump on manual scroll
 */
...
(function() {
  ...
  var toc_body = null;
  var toc_floatingBtn = null;
  var toc_progressBar = null;
  var toc_isCollapsed = toc_config.startCollapsed;
  var toc_isClickScrolling = false;
  var toc_clickScrollTimeout = null;
  var toc_lastScrollY = 0;

  // Utility: Generate unique ID
  function toc_generateId(text, index) {
  ...
  function toc_scrollTo(element) {
    var offset = toc_config.scrollOffset;
    var elemRect = element.getBoundingClientRect();
    var scrollTop = window.pageYOffset + elemRect.top - offset;

    window.scrollTo({
      top: scrollTop,
      behavior: toc_config.smoothScroll ? 'smooth' : 'auto'
    });
  }

  // Utility: Throttle function
  function toc_throttle(func, limit) {
    var inThrottle;
    return function() {
      var args = arguments;
      var context = this;
  ...
  function toc_build() {
    ...
    var postBody = toc_container.closest('.post-body') ||
                   toc_container.closest('.entry-content') ||
                   toc_container.closest('article') ||
                   document.querySelector('.post-body') ||
                   document.querySelector('.entry-content') ||
                   document.body;

    var allHeadings = postBody.querySelectorAll(toc_config.headingSelectors);
    toc_headings = [];

    for (var i = 0; i < allHeadings.length; i++) {
      var h = allHeadings[i];
      if (h.closest('#' + toc_config.containerId)) continue;
      toc_headings.push(h);
    }
    ...
    for (var j = 0; j < toc_headings.length; j++) {
      var heading = toc_headings[j];
      if (!heading.id) {
        heading.id = toc_generateId(heading.textContent, j);
      }
    }
  ...
  // Render TOC HTML
  function toc_render() {
    var headingAttr = toc_container.getAttribute('data-toc-heading');
    var titleText = headingAttr || toc_config.headingText;
    ...
    for (var i = 0; i < toc_headings.length; i++) {
      ...
      var itemClass = 'toc-item toc-item-' + tag;
      listHtml +=
        '<li class="' + itemClass + '">' +
          '<a href="#' + id + '" class="toc-link" data-target="' + id + '" data-number="' + number + '">' +
    ...
    var headerHtml =
      '<div class="toc-header">' +
        ...
        '</div>' +
        (toc_config.collapsible ?
          '<button class="toc-toggle" onclick="toc_toggleCollapse()" aria-label="Toggle TOC">' +
            '<span class="toc-toggle-icon">▼</span>' +
          '</button>' : '') +
      '</div>';

    var bodyHtml =
      '<div class="toc-body' + (toc_isCollapsed ? ' toc-collapsed' : '') + '" id="toc-body">' +
        '<ul class="toc-list">' + listHtml + '</ul>' +
      '</div>';

    var progressHtml = toc_config.showProgress ?
      '<div class="toc-progress">' +
        '<div class="toc-progress-bar" id="toc-progress-bar"></div>' +
      '</div>' : '';

    toc_container.innerHTML = headerHtml + bodyHtml + progressHtml;
    toc_container.className = 'toc-container';

    toc_body = document.getElementById('toc-body');
    toc_progressBar = document.getElementById('toc-progress-bar');

    toc_bindClicks();
  }

  // Bind click events
  function toc_bindClicks() {
    ...
    for (var i = 0; i < links.length; i++) {
      links[i].addEventListener('click', function(e) {
        ...
        if (toc_clickScrollTimeout) clearTimeout(toc_clickScrollTimeout);

        var prevActive = toc_container.querySelector('.toc-active');
        if (prevActive) prevActive.classList.remove('toc-active');
        this.classList.add('toc-active');
        toc_activeId = targetId;

        toc_scrollTo(targetEl);

        if (history.pushState) {
          history.pushState(null, null, '#' + targetId);
        }

        toc_clickScrollTimeout = setTimeout(function() {
          toc_isClickScrolling = false;
        }, 800);
      });
    }
  }
  ...
  // Toggle collapse
  window.toc_toggleCollapse = function() {
    toc_isCollapsed = !toc_isCollapsed;
    var toggleBtn = toc_container.querySelector('.toc-toggle');

    if (toc_body) {
      toc_body.classList.toggle('toc-collapsed', toc_isCollapsed);
    }
    if (toggleBtn) {
      toggleBtn.classList.toggle('toc-collapsed', toc_isCollapsed);
    }
  };

  // Scroll TOC item into view WITHOUT scrolling the page
  function toc_scrollItemIntoView(item) {
    if (!toc_body || !item) return;

    var bodyRect = toc_body.getBoundingClientRect();
    var itemRect = item.getBoundingClientRect();

    // Only scroll if item is outside visible area of TOC body
    var itemTop = itemRect.top - bodyRect.top;
    var itemBottom = itemTop + itemRect.height;
    var bodyHeight = toc_body.clientHeight;

    if (itemTop < toc_body.scrollTop) {
      toc_body.scrollTop = itemTop - 10;
    } else if (itemBottom > toc_body.scrollTop + bodyHeight) {
      toc_body.scrollTop = itemBottom - bodyHeight + 10;
    }
  }

  // Update active heading on scroll
  function toc_updateActive() {
    if (toc_isClickScrolling) return;

    var scrollPos = window.pageYOffset + toc_config.scrollOffset + 50;
    var found = null;

    for (var i = toc_headings.length - 1; i >= 0; i--) {
      if (toc_headings[i].offsetTop <= scrollPos) {
    ...
    if (found && found.id !== toc_activeId) {
      ...
      var newActive = toc_container.querySelector('[data-target="' + found.id + '"]');
      if (newActive) {
        newActive.classList.add('toc-active');

        // Only auto-scroll TOC item if user is NOT manually scrolling
        var scrollDelta = Math.abs(window.pageYOffset - toc_lastScrollY);
        if (scrollDelta < 50) {
          toc_scrollItemIntoView(newActive);
        }
      }
    ...
    toc_lastScrollY = window.pageYOffset;

    // Update progress bar
    if (toc_progressBar && toc_headings.length > 0) {
      var docHeight = document.documentElement.scrollHeight - window.innerHeight;
      var scrollPercent = docHeight > 0 ? (window.pageYOffset / docHeight) * 100 : 0;
      toc_progressBar.style.width = Math.min(100, Math.max(0, scrollPercent)) + '%';
    }

    // Show/hide floating button
    if (toc_floatingBtn && toc_container) {
      var tocRect = toc_container.getBoundingClientRect();
      var showFloating = tocRect.bottom < 0;
      toc_floatingBtn.classList.toggle('toc-visible', showFloating);
    }
  }

  // Create floating button
  function toc_createFloatingButton() {
    if (!toc_config.floatingButton) return;

    toc_floatingBtn = document.createElement('button');
    toc_floatingBtn.className = 'toc-floating';
    toc_floatingBtn.innerHTML = '☰<span class="toc-floating-badge">' + toc_headings.length + '</span>';
    toc_floatingBtn.setAttribute('aria-label', 'Open Table of Contents');
    toc_floatingBtn.onclick = function() {
      toc_container.scrollIntoView({ behavior: 'smooth' });
      if (toc_isCollapsed) {
        toc_toggleCollapse();
      }
    };

    document.body.appendChild(toc_floatingBtn);
  }
  ...
  function toc_init() {
    if (!toc_build()) return;
    toc_render();
    toc_createFloatingButton();

    window.addEventListener('scroll', toc_throttle(toc_updateActive, 150));

    setTimeout(toc_updateActive, 100);
  }
